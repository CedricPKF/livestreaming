...
void readarg(int argc,char *argv[])
{
	...
	REP(i,0,argc-1)
	{
		...
		if(i<argc-1 && !strcmp(argv[i], "-friend"))
		{
			strcpy(F_FRIEND, argv[i+1]);
		}
		
		if(i<argc-1 && !strcmp(argv[i], "-item"))
		{
			strcpy(F_ITEM, argv[i+1]);
		}
	}
}
...
int getitemid(string x)
{
	if(itemid[x]) return itemid[x];
...
void addedge2(string xx,string yy)
{
	...
	if(cid2>=C && !itemid[xx]) return; //TODO: cut
	if(!userid[yy]) return;

	int x = cmap[getitemid(xx)];
	int y = userid[yy];
...
void readuser()
{
	...
	FILE *f = fopen(F_FRIEND, "r");
	if(!f) exit("cannot read user file.");
	while(~fscanf(f, "%s %s",a,b))
	{
		addedge(a,b);
	}
	fclose(f);
...
void readitem()
{
	...
	FILE *f = fopen(F_ITEM, "r");
	if(!f) exit("cannot read item file.");
	while(~fscanf(f, "%s %s", a, b))
	{
		uid = userid[a];
	...
	int g = cid2/C;
	REP(i,1,cid2) cmap[i] = min(C, (i-1)/g+1);
	cid2 = C;

	for(auto i:edges) addedge2(i.F, i.S);
}
void add_edges()
{
